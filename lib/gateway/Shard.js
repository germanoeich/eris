    ...
    */
    ...
    identify() {
        var identify = {
            ...
            properties: {
                "os": process.platform,
                "browser": "Eris Dyno (https://dyno.gg)",
                "device": "Eris Dyno (https://dyno.gg)"
            }
        };
        ...
        if(this.client.options.preIdentify) {
            this.client.options.preIdentify(identify).then(() => {
                this.sendWS(OPCodes.IDENTIFY, identify, true);
                this.client.emit("shardIdentify", this.id);
            }).catch(err => {
    ...
    wsEvent(packet) {
        if (this.client.enabledGuilds.size || this.client.uncachedGuilds.size) {
            if (packet.d.guild_id && ( this.client.uncachedGuilds.has(packet.d.guild_id) ||
                (this.client.enabledGuilds.size && !this.client.enabledGuilds.has(packet.d.guild_id)) ) ) {
                    const guild = this.client.guilds.get(packet.d.guild_id);
                    if (guild) {
                        this.client.guilds.delete(packet.d.guild_id);
                    }
                    return;
            }
            ...
            if (packet.d.channel_id && !packet.d.guild_id) {
                ...
                if (!channel && this.client.channelGuildMap[packet.d.channel_id]) return;

                if (channel && channel.guild && ( this.client.uncachedGuilds.has(channel.guild.id) ||
                    (this.client.enabledGuilds.size && !this.client.enabledGuilds.has(channel.guild_id))) ) {
                        this.client.emit('debug', `ignoring ${channel.guild.id}`);
                        if (this.client.guilds.has(channel.guild.id)) {
                            this.client.guilds.delete(channel.guild.id);
                        }
                        return;
                }
            }
        ...
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                var userId = this.client.user ? this.client.user.id : null;
                if(packet.d.user.bot && packet.d.user.id !== userId) break;
            ...
            case "MESSAGE_CREATE": {
                ...
                if(channel) { // MESSAGE_CREATE just when deleting o.o
                    ...
                    */
                    var guild = this.client.guilds.get(packet.d.guild_id);
                    if(guild && guild.cacheMessages != undefined) {
            ...
            case "GUILD_UPDATE": {
                ...
                if (packet.d.name !== guild.name) update = true;
                if (packet.d.region !== guild.region) update = true;
                if (packet.d.icon !== guild.icon) update = true;
                if (packet.d.ownerID !== guild.ownerID) update = true;
                if (packet.d.verificationLevel !== guild.verificationLevel) update = true;
                ...
                if(packet.d.type === undefined || packet.d.type === 1) {
                    if(this.id === 0) {
                        ...
                        */
                        ...
                        this.emit("channelCreate", new PrivateChannel(packet.d, this.client));
            ...
            case "GUILD_MEMBERS_CHUNK": {
                ...
                if(this.getAllUsersCount.hasOwnProperty(guild.id)) {
                    if(this.getAllUsersCount[guild.id] <= 1) {
                        ...
                        this.checkReady();
                        this.emit('guildMemberChunkingComplete', guild.id);
                    } else {
    ...
    onWSMessage(packet) {
        ...
        if (this.listeners("rawCounter").length > 0 || this.client.listeners("rawCounter").length) {
            /**
            * Fired when the shard receives a websocket packet
            * @event Client#rawWS
            * @prop {Number} count The event count
            * @prop {Number} id The ID of the shard
            */
           this.emit("rawCounter", 1, this.id);
        }
    ...
    editStatus(status, game) {
        ...
        this.client.guilds.forEach((guild) => {
            if(guild.shard.id === this.id) {
                if (guild.members.has(this.client.user.id)) {
