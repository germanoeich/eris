...
class Member extends Base {
    ...
    update(data) {
        this._status = data.status !== undefined ? statusMap[data.status] : statusMap[this.status] || statusMap["offline"];
        // this.status = statusMap[data.status !== undefined ? data.status : this.status || "offline"];
        ...
        if(data.channel_id !== undefined) {
            this.voiceState.channelID = data.channel_id;
            this.voiceState.sessionID = data.channel_id === null ? null : data.session_id;
        }
        //  else if(this.voiceState.channelID === undefined) {
        //     this.voiceState.channelID = this.voiceState.sessionID = null;
        // }
    ...
    get status() {
        if (this._status != undefined) {
            return Object.keys(statusMap).find(key => statusMap[key] === this._status);
        }
        return "offline";
    }
